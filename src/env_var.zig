...
const EnvVarType = union(enum) {
    string: struct {
        ...
        // Cache struct factory - each environment variable gets its own Cache type with unique storage
        fn Cache(comptime ip: Input) type {
            return struct {
                const PtrType = ?[*]const u8;
                const LengthType = usize;

                /// Indicates an environment variable hasn't been loaded yet.
                const unknown_sentinel: LengthType = std.math.maxInt(LengthType);
                ...
                var value_len: std.atomic.Value(LengthType) = .init(std.math.maxInt(LengthType));

                /// Get the cached value.
                inline fn getCached() Output {
                    // This must be acquire so that subsequent loads of `value` see the value stored
                    // prior to the release store of `value_len` in reload().
                    const len = value_len.load(.acquire);
                    if (len == unknown_sentinel) {
                        return .{ .unknown = {} };
                    }
                ...
                pub inline fn reload(raw_env: ?[]const u8) ?NonOptionalReturnType {
                    ...
                    if (raw_env) |ev| {
                        ...
                        // This must be a release store to prevent reordering of the
                        // value.store(ev.ptr, .monotonic) statement.
                        //
                        // If it was monotonic, then other threads may not observe `value` update.
                        value_len.store(ev.len, .release);
                    } else {
                        value.store(null, .monotonic);
                        // This must be a release store to prevent reordering of the
                        // value.store(null, .monotonic) statement.
                        //
                        // If it was monotonic, then other threads may not observe `value` update.
                        value_len.store(0, .release);
                    }
    ...
    boolean: struct {
        ...
        pub fn stringIsTruthy(s: []const u8) bool {
            ...
            std.debug.print("[DEBUG env_var] stringIsTruthy({s}), length={}\n", .{ s, s.len });

            inline for (false_values) |tv| {
                std.debug.print("[DEBUG env_var]   comparing to ({s}), length={}, eql={}\n", .{ tv, tv.len, std.mem.eql(u8, s, tv) });
                if (std.mem.eql(u8, s, tv)) {
                    std.debug.print("[DEBUG env_var]   MATCH! returning false\n", .{});
                    return false;
                }
            }

            std.debug.print("[DEBUG env_var]   NO MATCH, returning true\n", .{});
            return true;
        }
