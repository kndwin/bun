...
pub const WinDir = if (bun.Environment.isWindows) struct {
    ...
    pub fn query(allocator: std.mem.Allocator) bun.sys.Maybe(Self) {
        ...
        switch (rc) {
            ...
            .result => |r| {
                if (r <= wchar_buf.len) {
                    var path = Path.AutoAbsPath.init();
        ...
        switch (rc) {
            ...
            .result => |r| {
                if (r <= wchar_buf.len) {
                    ...
                    path.append(wchar_buf[0..@as(usize, @intCast(r - 1))]);
                    return .initResult(.{ .#path = .{ .managed = path } });
                }
                ...
                switch (rc2) {
                    ...
                    .result => |r2| {
                        if (r2 <= wchar_buf.len) {
                            var path = Path.AutoAbsPath.init();
                            // -1 because we don't want the null terminator.
                            path.append(wchar_buf[0..@as(usize, @intCast(r2 - 1))]);
                            return .initResult(.{ .#path = .{ .managed = path } });
                        }
                    },
                }
...
const Path = @import("../paths/Path.zig");
